...
desplegada y `BOOTSTRAP_PASSWORD_SECRET_ID` igual al ARN/ID del secreto. La
plantilla lista para pegar en SSM, incluido `AWS_REGION` y el procedimiento de
reset seguro, está en `docs/runbook-development.md`. El script exige el
`runtime.env` generado por el despliegue, exige `APP_ENV=development`, fija el
correo autorizado y entrega únicamente el ID del secreto al contenedor. En
development use el secreto Terraform existente
