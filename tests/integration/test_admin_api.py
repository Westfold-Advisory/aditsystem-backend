...
async def _create_amigo_persona(
    client: AsyncClient, enlace_token: str, parent_id: str
) -> str:
...
async def test_create_user_as_admin(integration_client: AsyncClient) -> None:
    ...
    persona_id = await create_persona_without_auth(PersonRole.COORDINADOR_GENERAL)
...
async def test_create_user_duplicate_email_conflicts(
    ...
    first_persona_id = await create_persona_without_auth(PersonRole.COORDINADOR_GENERAL)
    second_persona_id = await create_persona_without_auth(
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_create_user_rejects_amigo_persona(
    integration_client: AsyncClient,
) -> None:
    admin_token = await login(integration_client, SEED_ADMIN_EMAIL)
    enlace_token = await login(integration_client, SEED_ENLACE_EMAIL)
    enlace_id = await persona_id_for(SEED_ENLACE_EMAIL)
    amigo_id = await _create_amigo_persona(
        integration_client, enlace_token, str(enlace_id)
    )
